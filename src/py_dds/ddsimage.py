...
from typing import List, Dict, Optional, Callable, Any, Union, Type, Tuple
from .ddsformat import DDSFormat
from .ddsexception import DDSException
...
class DDSImage:
    ...
    def __get_uncompressed_mip_maps_and_color_info(self, width: int, height: int) -> None:
        ...
        for _ in range(0, self.__mip_count):
            self.__mips.append(_DDSMip(width, height, offset))
    ...
    def __get_compressed_mip_maps(self, width: int, height: int, chunk_size: int) -> None:
        ...
        for _ in range(0, self.__mip_count):
            self.__mips.append(_DDSMip(width, height, offset))
            offset += math.ceil(width / 4) * math.ceil(height / 4) * chunk_size
            width = int(width / 2)
            height = int(height / 2)
